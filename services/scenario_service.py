...
def factories():
    return read_csv("factory_coordinates.csv")


def product_mapping():
    return read_csv("product_factory_mapping.csv")
...
def scenario_predictions(product, region, ship_mode, order_year, order_month):
    ...
    for factory in factories()["factory"]:
        ...
        rows.append({
            "factory": factory,
            "predicted_recorded_lead_time_days": float(model.predict(X)[0]),
            "current_factory": factory == current,
        })
    ...
    fallback = type(model).__name__ == "HistoricalScenarioProxy"
    out["evidence_type"] = (
        "Historical baseline proxy — factory effect not identifiable"
        if fallback else
        "Model-based counterfactual proxy"
    )
    out["confidence_score"] = 0.10 if fallback else 0.25
    return out, current, fallback
