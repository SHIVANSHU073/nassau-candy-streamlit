...
fig = px.bar(
    out.sort_values('predicted_recorded_lead_time_days'),
    x='factory',
    y='predicted_recorded_lead_time_days',
    title='Predicted recorded lead time by hypothetical factory'
)
...
if fallback:
    st.warning(
        'No factory-specific what-if effect is shown. The fallback model deliberately '
        'returns the historical baseline because the source data does not identify a causal factory effect.'
    )
else:
    selected = out.sort_values('predicted_recorded_lead_time_days').iloc[0]
    st.write(
        f"Lowest model-based scenario: **{selected['predicted_recorded_lead_time_days']:.1f} days** "
        f"at **{selected['factory']}**"
    )

st.caption(
    'Scenario values are not operational commitments. Validate capacity, transportation cost, '
    'production constraints and the source date logic before acting on them.'
)
